...
def calculate_min_max_index(event_data,first_sample,delta_sample,top_window,bottom_window,num_sample):
    ...
    if np.isnan(event_data).all():
        skip = 1
        return min_time_index, max_time_index, skip, top_window, bottom_window

    min_time = np.nanmin(event_data) 
    max_time = np.nanmax(event_data) 
    ...
    min_time_index = int(np.round((min_time / delta_sample)  - first_sample)) 
    if min_time_index<0: min_time_index = first_sample
    max_time_index = int(np.round((max_time / delta_sample)  - first_sample))
...
def get_picks_data(event_data,first_sample,delta_sample,min_time_index):
    picks_data = (np.round(event_data / delta_sample - first_sample)).astype(int) 
...
def create_grid(cube_shape):
    ii,jj = np.meshgrid(range(cube_shape[0]), range(cube_shape[1]), sparse=False, indexing='ij')
    kk = np.arange(cube_shape[2])
...
def calcualte_cube_sigma(picks_data,ii,jj,kk,top_window, bottom_window):
    cube_sigma = np.where((kk<=picks_data[ii,jj]+bottom_window) & (kk>=picks_data[ii,jj]-top_window), 1, 0)
    return cube_sigma

# @njit
def calculate_cube_nan(cube_sigma, vtdata):
    cube_nan = np.where(cube_sigma ==0, np.nan, vtdata)
    return cube_nan
...
def calculate_rms(cube_nan):
    amp = np.sqrt(np.nanmean(np.square(cube_nan), axis = 2))
    return amp

# @njit
def calculate_avg(cube_nan):
    amp = np.nanmean(cube_nan, axis=2)
    return amp

# @njit
def calculate_spa(cube_nan):
    # cube_pa = [(i > 0)*i for i in cube_nan]
    cube_pa = np.where(cube_nan>0, cube_nan, 0)
    amp = np.nansum(cube_pa, axis=2)
    del cube_pa
    return amp
...
def calculate_sna(cube_nan):
    ...
    cube_na = np.where(cube_nan<0, cube_nan, 0)
    amp = np.nansum(cube_na, axis=2)
    del cube_na
    return amp

# @njit
def calculate_saa(cube_nan):
    cube_aa = [np.abs(i) for i in cube_nan]
    amp = np.nansum(cube_aa, axis=2)
    del cube_aa
    return amp
...
def PExtract (cube: GeoIoVolume, horizons: GeoIoHorizonCollection, 
    ...
    n = int(np.round(num_i/number_of_i_patches))
    m = int(np.round(num_j/number_of_j_patches))
    event_counter = 0
    extracted_horizons = np.empty((len(events),num_i,num_j))
    ...
    return PExtractInternal(events, num_i, num_j, number_of_i_patches, number_of_j_patches, horizons, cube, method,
                     first_sample, delta_sample, num_sample, n, m, extracted_horizons, top_window, bottom_window, event_counter)


def PExtractInternal(events, num_i, num_j, number_of_i_patches, number_of_j_patches, horizons, cube, method, first_sample,
                     delta_sample, num_sample, n, m, extracted_horizons, top_window, bottom_window, event_counter):
    
    for e in range(len(events)):

        patched_horizon = np.empty((num_i,num_j))
        nan_horizon = np.ones((num_i,num_j))
        min_time_index = 0
        max_time_index = 0
        ...
        for i in range(number_of_i_patches):
            for j in range(number_of_j_patches):
                ...
                result = dask.delayed(process_internal)(skip, nan_horizon, patched_horizon, q, k, r, l, event_data, max_time_index, min_time_index,
                                cube, method, first_sample, delta_sample, top_window, bottom_window, num_sample)
...
def process_internal(skip, nan_horizon, patched_horizon, q, k, r, l, event_data, max_time_index, min_time_index, cube, method, first_sample,
                    delta_sample, top_window, bottom_window, num_sample):

    if skip == 1:
        nan_horizon[q:k, r:l] = np.nan
        patched_horizon[q:k, r:l] = np.nan
        return
    ...
    if method in ['inst', 'instant', 'ins', 'instantaneous']:
        amp = np.squeeze(vtdata[ii, jj, picks_data[ii, jj]], axis=2)
        patched_horizon[q:k, r:l] = amp
    elif method in ['rms']:
        patched_horizon[q:k, r:l] = calculate_rms(
            calculate_cube_nan(calcualte_cube_sigma(picks_data, ii, jj, kk, top_window, bottom_window), vtdata))
    elif method in ['average', 'avg', 'ave', 'mean']:
        patched_horizon[q:k, r:l] = calculate_avg(
            calculate_cube_nan(calcualte_cube_sigma(picks_data, ii, jj, kk, top_window, bottom_window), vtdata))
    elif method in ['spa']:
        patched_horizon[q:k, r:l] = calculate_spa(
            calculate_cube_nan(calcualte_cube_sigma(picks_data, ii, jj, kk, top_window, bottom_window), vtdata))
    elif method in ['sna']:
        patched_horizon[q:k, r:l] = calculate_sna(
            calculate_cube_nan(calcualte_cube_sigma(picks_data, ii, jj, kk, top_window, bottom_window), vtdata))
    elif method in ['saa']:
        patched_horizon[q:k, r:l] = calculate_saa(
            calculate_cube_nan(calcualte_cube_sigma(picks_data, ii, jj, kk, top_window, bottom_window), vtdata))
...
def S_INS (vt: GeoIoVolume, int_file: GeoIoHorizonCollection):
    ...
    nan_horizon = np.ones((num_i,num_j))
    new_picks = np.empty((len(event_names), num_i, num_j))
            ...
            for i in range(num_i):
                ...
                for j in range(num_j):
                    ...
                    if (not np.isnan(event_val)):
                        ...
                        k_val = np.round(event_val/digi - first_sample) 
                        if k_val<0: k_val = 0
